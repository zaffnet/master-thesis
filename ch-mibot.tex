...
As established in the previous chapter, recent advances in Large Language Models (LLMs) offer a promising avenue for automating talk therapy. Building on that foundation, this chapter details the design and deployment of MIBot, a fully generative, LLM-based chatbot for motivational interviewing (MI) with smokers. The work improves upon a predecessor system, MIBot v5.2~\citep{brown2023mi}, which, being partially scripted, had limitations in conversational naturalness. While the development and evaluation of the current MIBot system are described in detail by \citet{mahmood-etal-2025-fully}, this chapter complements that work by focusing on the technologies, design considerations, and implementation choices underlying the chatbot's deployment for a human feasibility study.
...
In summary, this chapter has detailed the iterative, clinician-informed design of the MIBot system, from its single-prompt architecture to its deployment as a robust, scalable, and secure cloud service. With the technical infrastructure in place, the next critical step is to evaluate its performance and safety with its intended users: human smokers. The following chapter describes the design of the human feasibility study conducted for this purpose.
