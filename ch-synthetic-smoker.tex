...
The transcript of the conversation between MIBot and the baseline synthetic smoker revealed two significant issues that made the synthetic smoker sound ``unnatural'' or atypical of a human smoker (see Appendix~\ref{app:baseline-smoker-transcript} for an example transcript):
...
\subsubsection{Methodology}
We modified the prompt to encourage a more natural, concise conversational style (see Appendix~\ref{app:verbosity-prompts} for the full prompts). Key additions to the prompt included:
...
\subsubsection{Methodology}
We hypothesized that resistance could be installed by providing the LLM with detailed backstories emphasizing different motivations and experiences (the full backstories are provided in Appendix~\ref{app:resistance-personas}). We created two distinct personas:
