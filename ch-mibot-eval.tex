...
This chapter presents a comprehensive evaluation of MIBot's effectiveness through a \margindex{feasibility study}feasibility study with 106 smokers recruited via the \margindex[feasibility study]{Prolific platform}Prolific platform. Building on the system design and implementation described in previous chapters, we assess MIBot's performance across four critical dimensions established in the smoking cessation and motivational interviewing literature: behavioural change readiness, \margindex[feasibility study]{perceived therapeutic empathy}perceived therapeutic empathy, \margindex[feasibility study]{adherence to MI principles}adherence to MI principles, and elicitation of client change talk.
...
Table~\ref{table:mibot_ruler_summary} summarizes the mean (and standard deviation) of each readiness ruler before the conversation, immediately afterwards, and one week later.   The table includes the change between the pre-conversation metric and one week later ($\Delta$). The \margindex[feasibility study]{Wilcoxon signed-rank test}Wilcoxon signed-rank test was applied to assess the significance of the change. Participants' confidence increased markedly from a baseline mean of 2.8 to 4.5 one week later ($\Delta=1.66$, $p<10^{-9}$). This represents a $\sim$59\% relative improvement and constitutes our \margindex[feasibility study]{primary outcome}primary outcome measure, aligning with MI theory that confidence (self-efficacy) is a key predictor of behaviour change \citep{Gwaltney2009-wj,Abar2013}. Importance also increased, albeit more modestly ($\Delta=0.5$, $p<0.005$), while readiness exhibited a small, non-significant change ($\Delta=0.23$, $p=0.22$).
...
Confidence changes varied across different smoking characteristics and quit history profiles. Participants were grouped by \margindex[feasibility study]{Heaviness of Smoking Index (HSI)}HSI (low 0--1, moderate 2--3, high 4--5, very high $\geq$ 6), daily cigarette consumption ($<$5, 5--9, 10--19, $\geq$20), whether they had made a quit attempt in the week before the study, and the number of prior attempts (0, 1--2, $\geq$3). The mean change in confidence for each subgroup is summarized in Table~\ref{tab:hsi_prequit}.
...
The system assigns behavioural codes to each utterance: counsellor utterances are classified as \margindex[AutoMISC]{MI-Consistent (MICO)}MI-Consistent (MICO), \margindex[AutoMISC]{MI-Inconsistent (MIIN)}MI-Inconsistent (MIIN), \margindex[AutoMISC]{Reflection (R)}Reflection (R), \margindex[AutoMISC]{Question (Q)}Question (Q), or \margindex[AutoMISC]{Other (O)}Other (O), while client utterances are categorized as \margindex[AutoMISC]{change talk (C)}change talk (C), \margindex[AutoMISC]{sustain talk (S)}sustain talk (S), or \margindex[AutoMISC]{neutral (N)}neutral (N). Following annotation of all utterances, we computed per-transcript summary metrics to quantify MI adherence. These comprise:
...
To provide a point of comparison for the MISC summary metrics, we also ran AutoMISC on the \margindex[AutoMISC]{HighLowQualityCounselling (HLQC) dataset}HighLowQualityCounselling (HLQC) dataset  \cite{perez-rosas-etal-2019-makes}, a publicly available\footnote{ \url{https://lit.eecs.umich.edu/downloads.html}} corpus of transcribed MI counselling demonstrations. The HLQC dataset comprises 155 high-quality (HLQC\_\textbf{HI}) and 104 low-quality (HLQC\_\textbf{LO}) transcripts sourced from public websites.
...
\margindex[user segmentation]{Entertained Sceptics}``Entertained Sceptics''---those who enjoyed but didn't find it helpful---often wanted more directive advice or concrete tools rather than exploratory conversation.
...
\begin{itemize}
\item \textbf{\margindex[feasibility study]{Effect size}Effect size}: Cohen's $d=0.71$ for confidence change exceeds typical digital intervention effects ($d=0.2$--0.4) \citep{Whittaker2016}
\item \textbf{\margindex[feasibility study]{MI fidelity}MI fidelity}: 95\% MI-consistent responses surpass typical human counsellor benchmarks (80--90\%) \citep{Moyers2016}
